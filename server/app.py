...
from init_scripts import init_multiple
from update_transactions import update_db 
...
db = client.db
to_initialize = ['all_users', 'content', 'payment']

current_block = -1

header_parser = reqparse.RequestParser()
header_parser.add_argument('address', required=True)
header_parser.add_argument('password', required=True)
...
class Login(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if rec_addr == address:
                user = db['all_users'].find_one({'address': address})
                if user is None:
                    res = db['all_users'].insert_one({'address': address, 'num_content': 0})
...
class Bio(Resource):
    ...
    def post(self):
        try:
            ...
            bio = args['bio']
            if verify_token(password) != address:
                return "Invalid password", 401
            try:
                res = db['all_users'].find_one_and_update(
                    {'address': address}, 
                    {"$set": {"bio": bio}}, 
                    upsert=False
                )
    ...
    def post(self):
        try:
            ...
            if verify_token(password) != address:
                return "Invalid password", 401
            try:
                user = db['all_users'].find_one_and_update(
                    {'address': address},
                    {"$inc": {'num_content': 1}},
                    upsert=False,
...
def has_access(address, artist_address):
    ...
    current_block = update_db(current_block, db['payment'])
...
class Artist(Resource):
    ...
    def get(self):
        ...
        user = db['all_users'].find_one({'address': artist_address})
        if user is None:
    ...
    def post(self):
        try:
            ...
            artist = db['all_users'].find_one({'address': artist_address})
            if artist is None:
                return {"bio": "No Bio Available", "songs": json.dumps({"songs": []})}, 200
